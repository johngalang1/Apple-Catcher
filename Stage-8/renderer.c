...
PARAMETERS: *msg - pointer to the message structure 
            *buffer - point to the screen buffer
...
void display_message(const message *msg, UINT16 *buffer) {
    ...
    while (msg->text[i] != '\0') {  /* Iterate through the text until the null terminator */
        ...
        if (bitmap) {  /* If a valid bitmap is found for the character */
            plot_char(buffer, msg->start_x + i * msg->spacing, msg->start_y, bitmap, CHARACTER_HEIGHT);
        }
...
*/
void clear_message(const message *msg, UINT16 *buffer) {
    int i = 0;
    while (msg->text[i] != '\0') {
        const UINT16 *bitmap = get_bitmap_for_char(msg->text[i]);
        
        if (bitmap) {
            clear_char(buffer, msg->start_x + i * msg->spacing, msg->start_y, CHARACTER_HEIGHT);
        }
...
NAME: render_pregame
PARAMETERS: 
    - UINT32 *front_buffer: Pointer to the front buffer.
    - UINT32 *back_buffer: Pointer to the back buffer.
    - model *curr_model: Pointer to the current game model.
PURPOSE: Prepares and renders all pre-game elements, including clearing the screen, rendering borders, and initializing timers and score displays.
DETAILS:
    - Clears both the front and back buffers.
    - Renders borders on both buffers.
    - Renders the start timer, round timer, and initial score display on both buffers.
*/
void render_pregame(UINT32 *front_buffer, UINT32 *back_buffer, model *curr_model) {
    ...
    /* Render borders on both buffers */
    render_borders((UINT16 *)front_buffer);
    render_borders((UINT16 *)back_buffer);

    /* Render the start timer, round timer, and score on the back buffer */
    ...
    /* Render the score on the front buffer as well */
